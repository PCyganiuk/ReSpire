...
Ekran edycji treningu w pierwotnym projekcie zawiera w górnej części tytuł, a w dolnej przycisk zapisu \textit{Save}, z którego w wersji ostatecznej zrezygnowano, ze względu na wprowadzenie mechanizmu automatycznego zapisu zmian. Główną część ekranu stanowi panel z trzema zakładkami:
\begin{itemize}
...
\newpage
\section{Wybór lokalnej bazy danych (\Karol)}

W dynamicznie ewoluującym świecie wytwarzania oprogramowania mobilnego wybór odpowiedniej warstwy przechowywania danych stanowi jedną z najbardziej fundamentalnych decyzji architektonicznych. W związku z potrzebą zapewnienia trwałości informacji między uruchomieniami aplikacji ReSpire --- w szczególności w celu przechowywania danych treningów i ustawień użytkownika --- konieczne było przeprowadzenie kompleksowej ewaluacji dostępnych technologii trwałego zapisu danych.

Specyfika pierwotnej wersji projektu ReSpire, jaką jest lokalne przechowywanie danych małego rozmiaru, stała się kluczowym filtrem, przez który analizowano potencjalne rozwiązania. W~przeciwieństwie do systemów klasy korporacyjnej wymagających synchronizacji w czasie rzeczywistym, priorytetami dla ReSpire są: szybkość uruchamiania, minimalizacja narzutu kodu oraz wydajność ładowania danych.
...
\subsection{Kryteria oceny i analiza porównawcza}

Jako parametry porównawcze przyjęto zestaw metryk obejmujący zarówno wskaźniki popularności, jak i aspekty ściśle techniczne: język implementacji, typ bazy, sposób przechowywania danych, język zapytań, wydajność na małych zbiorach danych, czas inicjalizacji, narzut kody konfiguracyjnego, bezpieczeństwo typów, łatwość migracji schematu, wpływ na rozmiar zbudowanej aplikacji oraz złożoność zapytań. Tabela \ref{tab:db_comparison} reprezentuje zestawienie ilościowe i jakościowe analizowanych technologii.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|c|c|c|c|}
        \hline
        \textbf{Parametr porównawczy} & \textbf{Hive} & \textbf{Isar} & \textbf{ObjectBox} & \textbf{\begin{tabular}{@{}c@{}}SQLite \\ (sqflite)\end{tabular}} & \textbf{Drift} & \textbf{Floor} \\
        \hline
        \begin{tabular}{@{}l@{}}Polubienia (pub.dev; \\ stan na 09.12.2025)\end{tabular} & \textbf{6,21 tys.} \cite{hive_pub} & 2,41 tys. \cite{isar_pub} & 1,52 tys. \cite{objectbox_pub} & 5,48 tys. \cite{sqflite_pub} & 2,27 tys. \cite{drift_pub} & 999 \cite{floor_pub} \\
        \hline
        \begin{tabular}{@{}l@{}}Pobrania (pub.dev; \\ stan na 09.12.2025)\end{tabular} & 1,06 mln \cite{hive_pub} & 8,12 tys. \cite{isar_pub} & 101 tys. \cite{objectbox_pub} & \textbf{2,39 mln} \cite{sqflite_pub} & 472 tys. \cite{drift_pub} & 20,4 tys. \cite{floor_pub} \\
        \hline
        \begin{tabular}{@{}l@{}}Popularność (Google Trends; \\ stan na 01.2025 w ujęciu \\ 31.01.2015 - 31.01.2025)\end{tabular} & \textbf{36} \cite{gtrends} & 6 \cite{gtrends} & 3 \cite{gtrends} & 7 \cite{gtrends} & 10 \cite{gtrends} & 16 \cite{gtrends} \\
        \hline
        \begin{tabular}{@{}l@{}}Gwiazdki (GitHub; \\ stan na 09.12.2025)\end{tabular} & \textbf{4,4 tys.} \cite{hive_git} & 4 tys. \cite{isar_git} & 1,2 tys. \cite{objectbox_git} & 3 tys. \cite{sqflite_git} & 3,1 tys. \cite{drift_git} & 1 tys. \cite{floor_git} \\
        \hline
        \hline
        Język & \textbf{Dart} & Dart+Rust & Dart+C & \textbf{Dart} & \textbf{Dart} & Dart+JS \\
        \hline
        Typ (Baza/ORM) & \textbf{Baza} & \textbf{Baza} & \textbf{Baza} & \textbf{Baza} & ORM & ORM \\
        \hline
        Sposób przechowywania & \textbf{Plikowy} &  \textbf{Plikowy} & \textbf{Plikowy} & SQLite & SQLite & SQLite \\
        \hline
        Model danych & \textbf{NoSQL} & \textbf{NoSQL} & \textbf{NoSQL} & Relacyjny & Relacyjny & Relacyjny \\
        \hline
        Język zapytań & \textbf{Dart} & \textbf{Dart} & \textbf{Dart} & SQL & Dart/SQL & SQL \\
        \hline
        Wydajność na małych danych & \textbf{Bardzo dobra} & \textbf{Bardzo dobra} & \textbf{Bardzo dobra} & Dobra & Dobra & Średnia \\
        \hline
        Czas inicjalizacji & \textbf{Błyskawiczny} & Szybki & Bardzo szybki & Umiarkowany & Umiarkowany & Wolniejszy \\
        \hline
        Narzut kodu konfiguracyjnego & \textbf{Niski} & Średni & Średni & Wysoki & Średni & Średni \\
        \hline
        Bezpieczeństwo typów & Częściowe & \textbf{Pełne} & \textbf{Pełne} & Brak & \textbf{Pełne} & \textbf{Pełne} \\
        \hline
        Migracja schematu & \textbf{Elastyczna} & Zaawansowana & \textbf{Automatyczna} & Trudna & Zaawansowana & \textbf{Automatyczna} \\
        \hline
        Wpływ na rozmiar APK & \textbf{Minimalny} & Wysoki & Wysoki & \textbf{Minimalny} & Średni & Średni \\
        \hline
        Złożoność zapytań & \textbf{Niska} & Wysoka & Wysoka & Wysoka & Wysoka & Średnia \\
        \hline
    \end{tabular}
    }
    \caption{Porównanie wybranych baz danych}
...
\subsection{Architektura oparta na pamięci operacyjnej a wydajność}
...
Hive prezentuje odmienne podejście architektoniczne. Jest to baza par klucz-wartość napisana całkowicie w języku Dart, która przy starcie aplikacji ładuje swoje indeksy (a w przypadku małych wolumenów danych - całą bazę) bezpośrednio do pamięci operacyjnej (RAM) urządzenia. Ponieważ ReSpire operuje na niewielkiej ilości informacji, cała baza danych mieści się w pamięci nawet na starszych urządzeniach. Dostęp do danych staje się więc operacją na pamięci RAM, co~jest rzędami wielkości szybsze niż operacje dyskowe.

\subsection{Minimalizm implementacyjny}

W kontekście małego zespołu deweloperskiego i ograniczonego czasu, złożoność implementacji ma kluczowe znaczenie.
\begin{itemize}
    \item \textbf{SQLite (sqflite):} Wymaga ręcznego pisania zapytań SQL oraz manualnego odwzorowywania wyników (\texttt{Map<String, dynamic>}) na obiekty Dart. Jest to proces podatny na błędy, które ujawniają się dopiero w trakcie działania aplikacji.
    \item \textbf{Drift/Floor:} Choć oferują bezpieczeństwo typów, wprowadzają znaczny narzut związany z~konfiguracją generatorów kodu i definicją tabel.
    \item \textbf{Hive:} Eliminuje ten narzut, oferując proste API i nie wymagając warstwy translacji między obiektami a tabelami. Zapis obiektu sprowadza się do jednej, prostej linii kodu, co dla małego projektu oznacza oszczędność wielu godzin pracy.
\end{itemize}

\subsection{Elastyczność schematu i migracje}
...
Przeprowadzona analiza porównawcza wskazuje, że \textbf{Hive} jest optymalnym wyborem dla aplikacji ReSpire. Łączy on w sobie dużą popularność z wydajnością idealnie dopasowaną do~modelu priorytetyzującego użytkowanie małych zbiorów danych offline. Hive zwycięża dzięki:
\begin{enumerate}
    \item \textbf{Wydajności:} Błyskawiczny dostęp do danych z pamięci RAM.
    \item \textbf{Prostocie:} Minimalizacja kodu konfiguracyjnego i brak konieczności odwzorowywania ORM.
    \item \textbf{Lekkości:} Czysty kod Dart bez skomplikowanych zależności, co skutkuje mniejszym rozmiarem aplikacji i łatwiejszym procesem budowania.
\end{enumerate}
Choć rozwiązania takie jak Isar czy ObjectBox oferują wyższą wydajność dla dużych zbiorów danych, a Drift potęgę relacyjnego SQL, w kontekście małej aplikacji stanowiłyby one nieuzasadniony \textit{overengineering} (pol. nadmierny narzut technologiczny).
