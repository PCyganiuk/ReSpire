...
\begin{itemize}
    \item \textit{Warstwa prezentacji} - wyświetla dane otrzymane poprzez warstwę logiki użytkownikowi oraz obsługuje żądania użytkownika. W ReSpire jest reprezentowana poprzez komponenty oraz składające się z nich strony.
...
System oparty jest na modularnym podziale, czyli składa się z mniejszych komponentów. Dzięki temu możliwe jest niezależne rozwijanie, testowanie, zmienianie i utrzymywanie poszczególnych modułów, a także ich wielokrotne użycie. Pozwala to również uporządkować kod i architekturę systemu. Poszczególne elementy warstwy prezentacji zostały wydzielone z widoków - na przykład strona treningu oddechowego posiada karuzelę z instrukcjami oraz animowane koło jako osobne komponenty. Łączy się to z zasadą \textit{Separation of concerns} (pol. podział zagadnień), gdzie każdy moduł skupia się na innym, określonym obszarze funkcjonalnym. Zmiany w jedym komponencie nie wpływają na pozostałe, co ułatwia zarządzanie nimi i wprowadzanie zmian.
...
Finalnie zrealizowany projekt graficznego interfejsu użytkownika składa się z 8 głównych, pełnoekranowych widoków oraz \textit{splash screen'a} (pol. ekranu powitalnego) z dodatkowymi wyskakującymi oknami dialogowymi.
