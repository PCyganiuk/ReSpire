...
Zadania Jakuba związane były z zaprojektowaniem i implementacją całej warstwy obsługi dźwięku w aplikacji. Kluczowym elementem jego pracy było stworzenie architektury silnika audio, opartej na komponentach \texttt{SoundManager}~(\ref{subsec:SoundManager}), \texttt{SingleSoundManager}~(\ref{subsec:SingleSoundManager}) oraz \linebreak\texttt{PlaylistManager}~(\ref{subsec:PlaylistManager}). Ponadto zaprojektował fundamentalne struktury danych~(\ref{img/class_diagram}) niezbędne do funkcjonowania aplikacji, wdrożył mechanizm internacjonalizacji~(\ref{sec:Languages}) oraz funkcjonalność pozwalającą użytkownikom na import własnych plików dźwiękowych.\todoJakub{Ref} Dbał również o stosowanie optymalnych rozwiązań, ponowne wykorzystanie istniejących komponentów oraz utrzymanie wysokiej jakości i przejrzystości kodu. Nadto odpowiadał za zarządzanie repozytorium na platformie GitHub, koordynację zadań oraz nadzór nad pracami zespołu.
