...
class TrainingController {
  ...
  ///
  /// This is kept as a field because _start() is also called after
  /// pause/resume. A local counter inside _start() would be reset
  /// every time the training is resumed.
  ...
  TrainingController(
    ...
    countingStep =
        parser.training.sounds.countingFrequencyMs.toInt();
  ...
  void _initializePreparationSound() async {
    ...
    second.value =
        max(1, (_remainingTime / 1000).ceil());
  ...
  void _start() {
    // _start() can be called both when training begins and after resume().
    // Synchronize the counting reference to the current training time so
    // counting sounds are not replayed immediately after resuming.
      ...
      Duration(milliseconds: _updateInterval),
        ...
        if (!end &&
            ...
            trainingTimeMs - _lastCountingTimeMs >=
                countingStep) {
          // Synchronize directly to the current time instead of
          // replaying every missed counting interval.
        ...
        // Countdown stays at 1 until the phase transition.
        second.value =
            max(1, (_remainingTime / 1000).ceil());
  ...
  void tryUpdateStageCounter() {
    ...
    if (_currentTrainingStageId != null) {
      ...
      if (playCycleSound && !_endingInitiated) {
        if (!_settings.breathingSoundEnabled) {
          _playShortSound(
            parser.training.sounds.cycleChangeSound.name,
          );
        }
