...
Konstruktor przyjmuje jako parametr obiekt \texttt{TrainingParser} (rozdział \ref{subsec:TrainingParser}) i zapisuje go do zmiennej \textit{parser}. Dodatkowo przy tworzeniu komponentu tworzone są obiekty \texttt{SoundManager}, \texttt{PlaylistManager} i \texttt{BinauralBeatGenerator} zajmujące się obsługą dźwięków (rozdział \ref{sec:Sounds}). Wyłączane jest także wyświetlanie etykiet progresu treningu, dopóki nie zostanie on rozpoczęty. Następnie ustawiane są wartości początkowe, przygotowywana jest faza rozpoczęcia i pobierane są pierwsze fazy treningu. 
...
\begin{lstlisting}[caption={TrainingController --- zakończenie funkcji start}, label={code/controller/end}]
  IF PozostalyCzas == 0 THEN
    LicznikFaz := LicznikFaz + 1

    IF LicznikFaz == 1 AND DzwiekiBinauralne THEN
        RozpocznijBinauralne(ObliczCzasTreningu())
    END IF
